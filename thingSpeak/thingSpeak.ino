...
String serverName =
    "https://api.thingspeak.com/update?api_key=BZ5TOKEQQGJOVVJA&field1=";
String WhatsappServer =
...
void whatsappAlert(int test){
    HTTPClient http;
    String serverPath = serverName + String(test);
    String whatsapp = WhatsappServer +"Reading+Alert:"+ String(test)+linkEnd;
    http.begin(whatsapp);
    int httpResponseCode = http.GET();
    if (httpResponseCode > 0) {
      Serial.print("HTTP Response code: ");
